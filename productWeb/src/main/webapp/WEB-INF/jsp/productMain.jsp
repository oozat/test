<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
<%@page import="model.ProductBeans, java.util.List" %>
<%

String msg=(String) request.getAttribute("msg");
List<ProductBeans> productList=(List<ProductBeans>) request.getAttribute("prductList");

%>
    
    
    
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>プロダクト登録</title>
</head>
<body>

<h1>プロダクト登録</h1>



<form action="/productWeb/CategoryServlet?action=register" method="post">

<select name="categoryId" size="10">
<% for(ProductBeans product:productList){%>
<option value="<%= product.getCategoryId()%>">カテゴリID<%= product.getCategoryId()%> 商品ID<%=product.getProductId()%>名前<%=product.getProductName() %>単価<%=product.getPrice() %></option>
<% } %>
</select>

<p>商品ID：<input type="text" name="categoryId"></p>
<p>商品名<input type="text" name="categoryName"></p>
<p>単価<input type="number" name="categoryName"></p>

<input type="submit" value="登録">
</form>







<form action="/productWeb/CategoryServlet?action=delete" method="post">

<select name="categoryId">
<% for(ProductBeans product:productList){%>
<option value="<%= product.getCategoryId()%>"><%= product.getCategoryId()%></option>
<% } %>
</select>

<p>カテゴリーID：<input type="text" name="categoryId"></p>
<p>カテゴリー名<input type="text" name="categoryName"></p>






<input type="submit" value="削除">
</form>









<% if(msg!=null){ %>
<p><%=msg %></p>
<% } %>

<select name="test" size="10">
<% for(ProductBeans product:productList){%>
<option><%= product.getCategoryId()%>:<%=product.getProductName() %></option>
<% } %>
</select>



</body>
</html>